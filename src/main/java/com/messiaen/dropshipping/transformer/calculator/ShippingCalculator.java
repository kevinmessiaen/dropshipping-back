...
public class ShippingCalculator {
    ...
    public ShippingMethodDto calculate(ShippingMethod method, BasketDto basket) {
        if (basket.getPrice() == null || method.getMinimumOrderValue() != null && method.getMinimumOrderValue() > basket.getPrice())
            return null;

        return ShippingMethodDto.builder()
                .id(method.getId())
                .name(method.getName())
                .fees((method.getOfferedOrderValue() == null ||
                        method.getOfferedOrderValue() > basket.getPrice()) ? method.getFees() : 0.0)
